...
const normalizeValue = (val) => {
  ...
  if (typeof val === "object") {
    if (val.image || val.url) return String(val.image || val.url).trim();
  ...
  try {
    ...
    questions.forEach((q) => {
      ...
      if (q.type === "single") {
        ...
        // 👉 lấy text thật sự
        const userTexts = userOrder.map((idx) => {
        ...
        const isCorrect =
          ...
          userTexts.every(
            (val, i) =>
              normalizeValue(val) === normalizeValue(correctTexts[i])
          );
        ...
        // correct là index theo vị trí
        const correctArray = Array.isArray(q.correct) ? q.correct : [];

        // userArray là index học sinh kéo vào
        const isCorrect =
          userArray.length === correctArray.length &&
          userArray.every((val, i) => Number(val) === Number(correctArray[i]));
        ...
        if (userArray.length === correctArray.length) {
          const isAllCorrect = userArray.every((val, i) => {
            // chuẩn hoá về cùng format
            const userVal = String(val).trim();       // "Đ" hoặc "S"
            const correctVal = String(correctArray[i]).trim();
        ...
        if (userAnswers.length === correctAnswers.length) {
          const isAllCorrect = correctAnswers.every(
            (correct, i) =>
              normalizeValue(userAnswers[i]) === normalizeValue(correct)
          );
